...
const account_index = $.getdata('youth_account_index') || ''
const $ = new Env("中青看点")
...
function GetCookie(){if($request&&$request.method!=`OPTIONS`&&$request.url.match(/\/NewTaskIos\/getTaskList/)){RefererVal=$request.headers.Referer;signheaderVal=RefererVal.match(/&uid=\d+/)+RefererVal.match(/&cookie=[_a-zA-Z0-9-]+/)+RefererVal.match(/&cookie_id=[a-zA-Z0-9]+/);if(signheaderVal)$.setdata(signheaderVal,'youthheader_zq'+account_index);$.log(`${$.name}获取Cookie${account_index}: 成功, signheaderVal: $}`);$.msg($.name,`获取Cookie${account_index}: 成功🎉`,``)}else if($request&&$request.method!=`OPTIONS`&&$request.url.match(/\/article\/info\/get/)){articlebodyVal=$request.url.split("?")[1];if(articlebodyVal)$.setdata(articlebodyVal,'read_zq'+account_index);$.log(`${$.name}获取阅读${account_index}: 成功, articlebodyVal: ${articlebodyVal}`);$.msg($.name,`获取阅读请求${account_index}: 成功🎉`,``)}else if($request&&$request.method!=`OPTIONS`&&$request.url.match(/\/v5\/user\/stay/)){const timebodyVal=$request.body;if(timebodyVal)$.setdata(timebodyVal,'readtime_zq'+account_index);$.log(`${$.name}获取阅读时长${account_index}: 成功, timebodyVal: ${timebodyVal}`);$.msg($.name,`获取阅读时长${account_index}: 成功🎉`,``)}else if($request&&$request.method!=`OPTIONS`&&$request.url.match(/\/withdraw\d?\.json/)){const withdrawVal=$request.body;const withdrawUrl=$request.url;if(withdrawVal)$.setdata(withdrawVal,'cashbody_zq'+account_index);if(withdrawUrl)$.setdata(withdrawUrl,'cashurl_zq');$.log(`${$.name}, 获取提现请求${account_index}: 成功, withdrawUrl: ${withdrawUrl}`);$.log(`${$.name}, 获取提现请求${account_index}: 成功, withdrawBody: ${withdrawVal}`);$.msg($.name,`获取提现请求${account_index}: 成功🎉`,``)}}
