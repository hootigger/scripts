...
if ($.isNode()) {
    ...
    for (let index = 1; index < 5; index++) {
        if ($.getdata('youthheader_zq' + index) === undefined || $.getdata('youthheader_zq' + index) === '') {
            break
        }
...
function GetCookie() {
    if ($request && $request.method != `OPTIONS` && $request.url.match(/\/TaskCenter\/(sign|getSign)/)) {
        ...
        if (signheaderVal) $.setdata(signheaderVal, 'youthheader_zq' + account_index)
        $.log(`${$.name} 获取Cookie: 成功,signheaderVal${account_index}: ${signheaderVal}`)
        $.msg($.name, `获取Cookies,ignheaderVal${account_index}: 成功🎉`, ``)
    }
    else if ($request && $request.method != `OPTIONS` && $request.url.match(/\/article\/complete/)) {
        const articlebodyVal = $request.body
        if (articlebodyVal) $.setdata(articlebodyVal, 'read_zq' + account_index)
        $.log(`${$.name} 获取阅读: 成功,articlebodyVal${account_index}: ${articlebodyVal}`)
        $.msg($.name, `获取阅读请求articlebodyVal${account_index}: 成功🎉`, ``)
    }
    else if ($request && $request.method != `OPTIONS` && $request.url.match(/\/v5\/user\/app_stay/)) {
        const timebodyVal = $request.body
        if (timebodyVal) $.setdata(timebodyVal, 'readtime_zq' + account_index)
        $.log(`${$.name} 获取阅读: 成功,timebodyVal${account_index}: ${timebodyVal}`)
        $.msg($.name, `获取阅读时长timebodyVal${account_index}: 成功🎉`, ``)
    }
    else if ($request && $request.method != `OPTIONS` && $request.url.match(/\/article\/red_packet/)) {
        const redpbodyVal = $request.body
        if (redpbodyVal) $.setdata(redpbodyVal, 'red_zq' + account_index)
        $.log(`${$.name} 获取惊喜红包: 成功,redpbodyVal${account_index}: ${redpbodyVal}`)
        $.msg($.name, `获取惊喜红包请求redpbodyVal${account_index}: 成功🎉`, ``)
    }
}
...
function Invitant2() {
    return new Promise((resolve, reject) => {
        $.post({
            url: `${YOUTH_HOST}User/fillCode`, headers: JSON.parse(signheaderVal), body: `{"code": "46746961"}`
        }, (error, response, data) => {
            // $.log(`Invitdata:${data}`)
        })
        resolve()
